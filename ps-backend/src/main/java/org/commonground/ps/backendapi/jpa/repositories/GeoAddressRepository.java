...
public interface GeoAddressRepository extends JpaRepository<GeoAddressEntity, Long> {
  ...
  @Query(value = "SELECT * FROM public.geo_address where street = :#{#street} and number = :#{#number}", nativeQuery = true)
  Optional<GeoAddressEntity> getAddressStreetAndNumber(@Param("street") String street, @Param("number") Integer number);

  @Query(value = "select row_number() OVER () id, sl.street, '' as type, true as active, '2000-01-01' as start_date, '2000-01-01' as end_date, null as government, '' as district, null as number, '' as letter, '' as addition, '' as postal, '' as city, 0 as srid, null as geo from (select distinct(street) as street from geo_address) sl order by SIMILARITY(lower(sl.street), lower(:#{#street})) desc limit 5", nativeQuery = true)
